...
function handleClick(gameState,rowIdx,colIdx){
    ...
    if (cell.mined){
    ...
    }else if (cell.mineCount>0 && !cell.spreaded){
        // 如果没有点开，设置为true
        let cell=gameState.cells[rowIdx][colIdx]
        if (!cell.spreaded){
            cell.spreaded=true
        ...
        let flaggedCount = countflaggedNeighbors(gameState,rowIdx,colIdx);
        if (flaggedCount === cell.mineCount){
            spreadSafeNighbors(gameState,rowIdx,colIdx);
        }
    }
...
function countflaggedNeighbors(gameState,rowIdx,colIdx){
    let flaggedCount = 0;
    for (let [drow,dcol] of directions){
        let newRowIdx = rowIdx + drow,newColIdx = colIdx + dcol;
        if (isValidCell(gameState,newRowIdx,newColIdx)&& gameState.cells[newRowIdx][newColIdx].flag){
            flaggedCount++;
        }
    }
    return flaggedCount
}
function spreadSafeNighbors(gameState,rowIdx,colIdx){
    for (let [drow,dcol] of directions){
        let newRowIdx = rowIdx +drow,newColIdx=colIdx+dcol;
        if(isValidCell(gameState,newRowIdx,newColIdx)){
            let cell = gameState.cells[newRowIdx][newColIdx];
            if(!cell.spreaded && !cell.flag) {
                handleClick(gameState, newRowIdx, newColIdx);
            }
        }
    }
}
function isValidCell(gameState, rowIdx, colIdx) {
    return rowIdx >= 0 && rowIdx < gameState.m && colIdx >= 0 && colIdx < gameState.n;
}
